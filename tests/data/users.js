export const plainUser = {
  id: '85193083-ce22-43a5-993b-1c7aba53d13c',
  username: 'plainuser',
  password: '123456789',
  albumId: 'feba0696-8954-4596-a849-0087cbe8ea76',
};

export const bannedUser = {
  id: 'b1786ac1-5cc8-4156-8471-8a80a87efe17',
  username: 'banneduser',
  password: '123456789',
};

export const adminUser = {
  id: '60bfb8a7-5754-4186-acd2-44b20ef32399',
  username: 'adminuser',
  password: '123456789',
};

export const editorUser = {
  id: '102e6ed9-cdac-4c9c-9483-a3309970db59',
  username: 'editoruser',
  password: '123456789',
};

export const writerUser = {
  id: '6750ff62-7195-49f5-bf81-e3d395e6cdcf',
  username: 'writeruser',
  password: '123456789',
};

export const otherUser = {
  id: '941b2ac5-2519-44de-84f9-ea9980e4631f',
  username: 'otheruser',
  password: '123456789',
};

export const pendingUser = {
  id: 'cc834ce6-58df-4381-aed3-8fe4c2923434',
  username: 'pendinguser',
  password: '123456789',
};

export const createdUser = {
  id: 'ef5965c0-306f-4902-b1da-cc1c3458cefd',
  username: 'createduser',
  password: '123456789',
  email: 'createduser@gmail.com',
};

export const createdOAuthUser = {
  id: '98bd65c0-306f-4902-b1da-cc1c34586754',
  username: 'createdOAuthuser',
  email: 'createdoauthuser@gmail.com',
  timezone: 'Europe/Paris',
  locale: 'pt-BR',
};

