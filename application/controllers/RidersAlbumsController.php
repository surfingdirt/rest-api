<?php
class RidersAlbumsController extends Api_Controller_Action
{
    ...
    {
    	$riderId = $this->_request->getParam('id');
		if(in_array($user->status, array(User::STATUS_EDITOR, User::STATUS_ADMIN)) || $user->getId() == $riderId){
			$return = '1';
		} else {
			$return  = $this->_table->getAdapter()->quoteInto('status = ?', Data::VALID);
		}
		
		$return .= $this->_table->getAdapter()->quoteInto(' AND submitter = ?', $riderId);
