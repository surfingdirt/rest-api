...
abstract class Data_Row extends Cache_Object_Row implements Data_Row_DataInterface
{
  ...
  {
    $textRow = null;
    $rowset = $this->getAllTranslatedTexts($lang, $write);
    ...
    /**
     * @todo: create new entries for the current language if none exist
     */
   ...
   */
  public function getAllTranslatedTexts($lang, $skipCache = true)
  {
    ...
    $table = new Data_TranslatedText();
    if ($translatedTexts === false || count($translatedTexts) == 0) {
      $translatedTexts = Data_TranslatedText::getAllTranslatedTexts($this->id, $this->getItemType());
  ...
  public function getComments(User_Row $user, Lib_Acl $acl)
  {
    ...
    // TODO: loop over comments to make sure that the user has the right to read them
    $ret = array();
    foreach ($comments as $comment) {
      if ($comment->isReadableBy($user, $acl)) {
        $ret[] = $comment;
      }
    }
  ...
  protected function _getCacheIdsForClear()
  {
    $return = array(
      ...
      $this->_getSubmitterCacheId(),
      $this->_getLastEditorCacheId(),
    );
  ...
  protected function _doInsert($setInValid = false)
  {
    ...
    if ($this->_arrTags && count($this->_arrTags)) {
      $this->_insertTags();
    }
