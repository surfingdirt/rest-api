...
class Data_Utils
{
  public static function getList(User_Row $user, Lib_Acl $acl, $dataType = null, $where = null, $itemsPerPage = DEFAULT_ITEMS_PER_PAGE, $orderByDate = true, $manageValidity = true)
  {
    if (empty($dataType)) {
    ...
    $select = $table->select();
    if (!$seeInvalidItems && $manageValidity) {
      $select->where("status = '" . Data::VALID . "' OR submitter = " . $user->getId());
    }
    if (!empty($where)) {
      $select->where($where);
    }
